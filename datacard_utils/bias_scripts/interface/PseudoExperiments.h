...
public:
  ...
  TH1* biasfitS(const TString& np) const {
    return getClone(getHist(biasPostfitS_,np));
  }
  ...
  Double_t biasfitSMean(const TString& np) const{
      return getHist(biasPostfitS_, np)->GetMean();
  }
  Double_t biasfitSMeanError(const TString& np) const{
      return getHist(biasPostfitS_, np)->GetMeanError();
  }
